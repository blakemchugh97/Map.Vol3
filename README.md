...
A first live IMSR integration exists as a **self-contained, master-flag-gated feature**. The entire surface is isolated in `js/imsr_live.js` (behind `IMSR_LIVE.enabled`), backed by `imsr-live.json`, and generated by `tools/imsr_build_live.py`; flipping the flag off makes the app behave exactly as before. The flag is **currently ON (enabled for review)**. It adds three tightly-scoped, additive capabilities: an experimental sit-rep thinning mode, subtle GACC preparedness-level tinting in the GACC-regions zone view, and a debug-only IMSR incident block for EXACT-tier matched incidents. None of them change ranking, NICC math, or crew membership, and every accessor fails safe to "nothing shown."
